...
func newConfigFromVyos(ctx context.Context, vyos_key *configKey, resource_schema *schema.Resource, vyos_client *client.Client) (*configBlock, error) {
	...
	*/
	...
	logger("DEBUG", "vyos_key: %#v", vyos_key)

	config_block := configBlock{
		key: vyos_key,
	}

	logger("DEBUG", "Asking client to fetch vyos config: %#v", config_block.key)
...
func vyosWalker(ctx context.Context, config_block *configBlock, resource_schema interface{}, parent_vyos_native_config interface{}) error {
	...
	logger("TRACE", "{%s} Walking", config_block.key.Key)

	if parent_vyos_native_config == nil {
		logger("TRACE", "{%s} parent_vyos_native_config is nil", config_block.key.Key)
		return nil
	}
	...
	case map[string]*schema.Schema:
		// Config block
		logger("TRACE", "resource_schema is map of schema")

		for key_string, parameter_schema := range resource_schema {
			// Convert to map as expected based on schema type
			logger("TRACE", "key_string: '%s' parent_vyos_native_config: '%#v'", key_string, parent_vyos_native_config)
			...
			if vyos_config, ok := parent_vyos_config[vyos_key_string]; ok {
				...
				logger("DEBUG", "parent_vyos_config does not contain key: %s", vyos_key_string)
	...
	case *schema.Schema:
		// Config parameters
		logger("TRACE", "resource_schema.Type: %s", resource_schema.Type)
		logger("TRACE", "parent_vyos_native_config: %#v", parent_vyos_native_config)
		...
		case schema.TypeString, schema.TypeInt, schema.TypeFloat:
			...
			if vyos_native_config, ok := parent_vyos_native_config.(string); ok {
				...
				logger("ERROR", "(key: %s)resource_schema is unhandled: %#v", config_block.key.Key, resource_schema)
		...
		case schema.TypeBool:
			...
			logger("TRACE", "Should be bool: parent_vyos_native_config: %#v", parent_vyos_native_config)
		...
		case schema.TypeList, schema.TypeMap, schema.TypeSet:
			...
			logger("TRACE", "resource_schema set/list")
			...
			if resource_schema_elem, ok := resource_schema.Elem.(*schema.Resource); ok {
				...
				logger("TRACE", "resource_schema_elem_schema: %#v", resource_schema_elem_schema)
				...
				if resource_schema.MaxItems != 1 {
					logger("ERROR", "resource_schema has elem indicating it is a config block, but does not have MaxItems set to one, this configuration is currently unhandled: %#v", resource_schema)
				...
				logger("TRACE", "resource_schema_elem: '%#v'", resource_schema_elem)
				...
				// Make unhandled cases visible
				logger("ERROR", "resource_schema.Elem is unhandled: %#v", resource_schema)
...
func terraformWalker(ctx context.Context, config_block *configBlock, resource_schema interface{}, terraform_native_config interface{}) error {
	...
	logger("TRACE", "parent_config.key: %#v", config_block.key)
	logger("TRACE", "resource_schema: %#v", resource_schema)
	...
	case map[string]*schema.Schema:
		...
		for key_string, parameter_schema := range resource_schema {
			...
			if terraform_sub_config, ok := terraform_config[key_string]; ok {
				logger("TRACE", "found key: '%s' in terraform_config: %#v", key_string, terraform_config)
				...
				if terraform_sub_config != "" && terraform_sub_config != nil {
					...
					logger("TRACE", "key: '%s' seems to be empty string or nil: %#v", key_string, terraform_sub_config)
				}
			} else {
				logger("TRACE", "terraform_config does not contain key: %s", key_string)
	...
	case *schema.Schema:
		// Config parameters
		logger("TRACE", "resource_schema.Type: %s", resource_schema.Type)
		logger("TRACE", "parent_terraform_native_config: %#v", terraform_native_config)
		...
		case schema.TypeInt:
			...
			if j, ok := terraform_native_config.(int); ok {
				...
				logger("ERROR", "(key: %s)resource_schema is unhandled: %#v, not int or int64", config_block.key.Key, resource_schema)
		...
		case schema.TypeFloat:
			if f, ok := terraform_native_config.(float64); ok {
				...
				logger("ERROR", "(key: %s)resource_schema is unhandled: %#v, not float", config_block.key.Key, resource_schema)
		...
		case schema.TypeString:
			...
			if terraform_native_config_str, ok := terraform_native_config.(string); ok {
				...
				// Make unhandled cases visible
				logger("ERROR", "(key: %s)resource_schema is unhandled: %#v, not string", config_block.key.Key, resource_schema)
				return fmt.Errorf("(key: %s)resource_schema is unhandled: %#v, not string", config_block.key.Key, resource_schema)
			}

		case schema.TypeMap:
			logger("ERROR", "(key: %s) TODO resource_schema.Type: %s", config_block.key.Key, resource_schema, resource_schema.Type)
		...
		case schema.TypeList, schema.TypeSet:
			...
			if resource_schema.Type == schema.TypeSet {
				logger("TRACE", "converting from set to list")
			...
			if resource_schema_elem, ok := resource_schema.Elem.(*schema.Resource); ok {
				...
				if resource_schema.MaxItems != 1 {
					...
					for key_string, parameter_schema := range resource_schema_elem_schema {
						...
						if terraform_sub_config, ok := terraform_config[key_string]; ok {
							...
							logger("TRACE", "could not find key: '%s' in terraform_config: %#v", key_string, terraform_config)
						}
					}
				}
			} else if resource_schema_elem, ok := resource_schema.Elem.(*schema.Schema); ok {
				...
				switch resource_schema_elem.Type {
				case schema.TypeBool, schema.TypeFloat, schema.TypeInt, schema.TypeString:
					logger("TRACE", "adding value: terraform_native_config: '%#v'", terraform_native_config)
	...
	logger("DEBUG", "err: %s, tf json data: %s\n", tf_err, tf_json_data)
